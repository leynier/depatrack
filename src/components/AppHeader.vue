...
import { SunIcon, MoonIcon, ChartBarIcon, UserIcon, Bars3Icon, GlobeAltIcon, LanguageIcon } from '@heroicons/vue/24/outline';
...
const getThemeText = () => {
  return theme.value === 'dark' ? t('theme.lightMode') : t('theme.darkMode');
};
</script>
  ...
  <header class="bg-background border-b border-border">
    <div class="max-w-7xl mx-auto px-6 py-4">
      <div class="flex items-center justify-between">
        <!-- Mobile: Hamburger + Title + Sync -->
        <div class="md:hidden flex items-center gap-3 flex-1">
          <!-- Mobile Menu (Left Side) -->
          <DropdownMenu v-model:open="showMobileMenu">
            ...
            <DropdownMenuContent align="start" class="w-48">
              <DropdownMenuItem @click="openGitHub">
                ...
                {{ t('navigation.githubShort') }}
              </DropdownMenuItem>
              <DropdownMenuItem @click="openStats">
                <ChartBarIcon class="h-4 w-4 mr-2" />
                {{ t('navigation.statistics') }}
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem @click="handleThemeToggle">
                <component :is="getThemeIcon()" class="h-4 w-4 mr-2" />
                {{ getThemeText() }}
              </DropdownMenuItem>
              <DropdownMenuItem @click="handleLanguageToggle">
                <LanguageIcon class="h-4 w-4 mr-2" />
          ...
          <div class="flex items-center">
            <h1 class="text-xl font-bold text-foreground sans-serif">{{ t('app.title') }}</h1>
            <SyncIcon />
          </div>
        </div>
